...
function Trainer:computeScore(output, target, nCrops)
   ...
   for i = 1,x:size(1) do
      eval1[target[i]][x[i][1]] = eva1l[target[i]][x[i][1]]+1
   end
...
function Trainer:computeScoreTest(output, target, nCrops)
   ...
   local x = predictions:narrow(2,1,1)
   for i = 1,x:size(1) do
      eval2[target[i]][x[i][1]] = eva2l[target[i]][x[i][1]]+1
   end
...
function Trainer:learningRate(epoch)
  ...
  elseif self.opt.dataset == 'cifar10' then --or self.opt.dataset =='lungroi' then
